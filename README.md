...
# 配置环境变量
cp .env.example .env
# 编辑 .env 文件，填入你的 OpenAI API Key: OPENAI_API_KEY=sk-...

# 启动基础设施 (Postgres, Redis, FastAPI, Celery)
docker-compose up -d --build
```
*API 服务将运行在 `http://localhost:8000`。MCP SSE 端点位于 `http://localhost:8000/mcp`。*

### 2. 启动前端

```bash
cd motifold-frontend
...
```text
motifold/
├── motifold-backend/      # 包含 FastAPI, Celery Workers, LangGraph 智能体、MCP 服务器及数据库迁移
└── motifold-frontend/     # 包含 Next.js 应用, UI 交互组件及 ECharts 图表可视化
```
