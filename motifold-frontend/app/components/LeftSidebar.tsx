...
export default function LeftSidebar() {
  ...
  useEffect(() => {
    ...
    } else if (searchParams.get('view') === 'blackboard') {
          ...
          <div className={`flex flex-col ${isChatsOpen ? 'flex-1 min-h-0' : 'flex-shrink-0'}`}>
            ...
            >
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest group-hover:text-slate-600 transition-colors">
                {searchParams.get('view') === 'matrix' ? `形态分析历史 (${morphologicalHistory.length})` : searchParams.get('view') === 'blackboard' ? `黑板历史 (${blackboardHistory.length})` : `对话 (${chats.length})`}
              </span>
            ...
            {isChatsOpen && (
              <div 
                ...
                onScroll={searchParams.get('view') !== 'matrix' && searchParams.get('view') !== 'blackboard' ? handleScroll : undefined}
              >
                {searchParams.get('view') === 'blackboard' ? (
                  <>
                    <button 
                    ...
                    <button 
                      onClick={() => router.push('/?chatId=new')}
          ...
          <div className={`flex flex-col ${isChatsOpen ? 'flex-shrink-0 max-h-[40%]' : 'flex-1 min-h-0'}`}>
            ...
            {isViewsOpen && (
              <div className="flex-1 overflow-y-auto space-y-1 mt-1 pr-1 custom-scrollbar">
                <button 
                  onClick={() => router.push('/?view=matrix')}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 ${searchParams.get('view') === 'matrix' ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
                >
                  <Network className={`w-4 h-4 flex-shrink-0 ${searchParams.get('view') === 'matrix' ? 'text-indigo-600' : 'text-slate-400'}`} />
                  <span className="font-medium">形态分析</span>
                </button>
                
                <button 
                  onClick={() => router.push('/?view=blackboard')}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 ${searchParams.get('view') === 'blackboard' ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
                >
                  <Presentation className={`w-4 h-4 flex-shrink-0 ${searchParams.get('view') === 'blackboard' ? 'text-indigo-600' : 'text-slate-400'}`} />
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <div className="px-6 py-4 bg-slate-50 flex justify-end gap-3 rounded-b-2xl">
              ...
              <button
                onClick={searchParams.get('view') === 'matrix' ? confirmDeleteMorphological : searchParams.get('view') === 'blackboard' ? confirmDeleteBlackboard : confirmDeleteChat}
