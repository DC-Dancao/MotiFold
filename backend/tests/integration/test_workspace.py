...
async def test_list_workspaces(auth_client):
    ...
    await auth_client.post("/api/workspaces", json={"name": "Test Workspace"})
    await auth_client.post("/api/workspaces", json={"name": "Second Workspace"})
...
async def test_delete_workspace(auth_client):
    ...
    ws_res = await auth_client.post("/api/workspaces", json={"name": "To Delete"})
    ws_id = ws_res.json()["id"]

    del_res = await auth_client.delete(f"/api/workspaces/{ws_id}")
    assert del_res.status_code == 200

    get_res = await auth_client.get(f"/api/workspaces/{ws_id}")
