...
async def test_create_chat_without_workspace(auth_client):
    ...
    res = await auth_client.post("/api/chats", json={})
...
async def test_create_chat_with_model(auth_client):
    ...
    res = await auth_client.post("/api/chats", json={"model": "max"})
    assert res.status_code == 200
    data = res.json()
...
async def test_create_chat_with_invalid_model(auth_client):
    ...
    res = await auth_client.post("/api/chats", json={"model": "invalid"})
...
async def test_get_chat(auth_client):
    ...
    # Create chat first
    create_res = await auth_client.post("/api/chats", json={})
    chat_id = create_res.json()["id"]

    # Get chat
    res = await auth_client.get(f"/api/chats/{chat_id}")
    assert res.status_code == 200
    data = res.json()
...
async def test_get_chat_not_found(auth_client):
    ...
    res = await auth_client.get("/api/chats/99999")
...
async def test_list_chats_default_limit(auth_client):
    ...
    for _ in range(5):
        await auth_client.post("/api/chats", json={})

    res = await auth_client.get("/api/chats")
    assert res.status_code == 200
    data = res.json()
    assert len(data) == 5
...
async def test_list_chats_with_pagination(auth_client):
    ...
    # Get first 2
    res = await auth_client.get("/api/chats?skip=0&limit=2")
    assert res.status_code == 200
    data = res.json()
    assert len(data) == 2

    # Get next 1
    res = await auth_client.get("/api/chats?skip=2&limit=2")
    assert res.status_code == 200
    data = res.json()
    assert len(data) == 1
...
async def test_delete_chat(auth_client):
    ...
    # Delete it
    res = await auth_client.delete(f"/api/chats/{chat_id}")
    assert res.status_code == 200

    # Verify it's gone
    get_res = await auth_client.get(f"/api/chats/{chat_id}")
    assert get_res.status_code == 404


async def test_delete_chat_not_found(auth_client):
    """Return 404 when deleting non-existent chat."""
    res = await auth_client.delete("/api/chats/99999")
    assert res.status_code == 404
...
async def test_create_chat_in_workspace_full_flow(auth_client, db_session):
    ...
    # Create workspace
    ws_res = await auth_client.post("/api/workspaces", json={"name": "Test Workspace"})
    assert ws_res.status_code == 200
    ws_id = ws_res.json()["id"]

    # Create chat in workspace
    chat_res = await auth_client.post("/api/chats", json={"workspace_id": ws_id})
    ...
    list_res = await auth_client.get(f"/api/chats?workspace_id={ws_id}")
...
async def test_create_chat_invalid_workspace(auth_client):
    ...
    res = await auth_client.post("/api/chats", json={"workspace_id": 99999})
    assert res.status_code == 404
...
async def test_get_messages_empty(auth_client):
    ...
    # Get messages
    msg_res = await auth_client.get(f"/api/chats/{chat_id}/messages")
    assert msg_res.status_code == 200
    data = msg_res.json()
    assert data == []
...
async def test_get_messages_with_pagination(auth_client):
    ...
    msg_res = await auth_client.get(f"/api/chats/{chat_id}/messages?skip=0&limit=10")
    assert msg_res.status_code == 200


async def test_get_messages_nonexistent_chat(auth_client):
    """Get messages from non-existent chat returns 404."""
    msg_res = await auth_client.get("/api/chats/99999/messages")
    assert msg_res.status_code == 404
...
async def test_stream_chat_endpoint_exists(auth_client):
    ...
    res = await auth_client.get(f"/api/chats/{chat_id}/stream")
...
async def test_stream_chat_nonexistent_chat(auth_client):
    ...
    res = await auth_client.get("/api/chats/99999/stream")
    assert res.status_code == 404
