...
async def test_create_chat_in_workspace(auth_client, db_session):
    ...
    ws_res = await auth_client.post("/api/workspaces", json={"name": "Chat Workspace"})
...
async def test_list_chats_by_workspace(auth_client):
    ...
    ws1_res = await auth_client.post("/api/workspaces", json={"name": "WS 1"})
    ws1_id = ws1_res.json()["id"]

    ws2_res = await auth_client.post("/api/workspaces", json={"name": "WS 2"})
    ws2_id = ws2_res.json()["id"]

    await auth_client.post("/api/chats", json={"workspace_id": ws1_id})
    await auth_client.post("/api/chats", json={"workspace_id": ws1_id})
    await auth_client.post("/api/chats", json={"workspace_id": ws2_id})

    res1 = await auth_client.get(f"/api/chats?workspace_id={ws1_id}")
    ...
    res2 = await auth_client.get(f"/api/chats?workspace_id={ws2_id}")
