...
def check_and_add_column(conn, schema: str, column: str) -> bool:
    """
    Check if column exists and add if missing, all within the same connection.
...
def run_add_solutions_mode_to_chats(self) -> dict:
    ...
    try:
        ...
        migration_id, current_status = row
        ...
        for schema in schemas:
            try:
                added = check_and_add_column(conn, schema, 'solutions_mode')
                if added:
